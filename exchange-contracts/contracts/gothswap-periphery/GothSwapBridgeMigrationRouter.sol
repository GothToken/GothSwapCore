...
import "../gothswap-lib/libraries/TransferHelper.sol";
import "./interfaces/IBridgeToken.sol";
import "./libraries/Roles.sol";
import "./libraries/GothSwapLibrary.sol";

contract GothSwapBridgeMigrationRouter {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'GothSwapBridgeMigrationRouter: EXPIRED');
        _;
    }

    // makes sure the admin is the one calling protected methods
    modifier onlyAdmin() {
        require(adminRole.has(msg.sender), 'GothSwapBridgeMigrationRouter: Unauthorized');
        _;
    }
    ...
    function addAdmin(address account) external onlyAdmin {
        require(account != address(0), "GothSwapBridgeMigrationRouter: Address 0 not allowed");
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(msg.sender != account, "GothSwapBridgeMigrationRouter: You can't demote yourself");
    ...
    function addMigrator(address tokenAddress, address migratorAddress) external onlyAdmin {
        require(tokenAddress != address(0), "GothSwapBridgeMigrationRouter: tokenAddress 0 not supported");
        require(migratorAddress != address(0), "GothSwapBridgeMigrationRouter: migratorAddress 0 not supported");
    ...
    function _allowToken(address tokenAddress, address spenderAddress) internal {
        IGothSwapERC20(tokenAddress).approve(spenderAddress, type(uint).max);
    }
    ...
    function _addLiquidity(
        ...
        (uint112 reserve0, uint112 reserve1,) = IGothSwapPair(pairToken).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = GothSwapLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
            quote0 = GothSwapLibrary.quote(amountIn1, reserve1, reserve0);
        }
        ...
        liquidityAmount = IGothSwapPair(pairToken).mint(to);
    }
    ...
    function _rescueLiquidity(
        ...
        (amountTokenA, amountTokenB) = IGothSwapPair(liquidityPair).burn(address(this));
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        require(pairA != address(0), "GothSwapBridgeMigrationRouter: liquidityPairFrom address 0");
        require(pairA != address(0), "GothSwapBridgeMigrationRouter: liquidityPairTo address 0");
        require(pairA != pairB, "GothSwapBridgeMigrationRouter: Cant convert to the same liquidity pairs");
        require(
            IGothSwapPair(pairA).token0() == IGothSwapPair(pairB).token0() ||
            IGothSwapPair(pairA).token0() == IGothSwapPair(pairB).token1() ||
            IGothSwapPair(pairA).token1() == IGothSwapPair(pairB).token0() ||
            IGothSwapPair(pairA).token1() == IGothSwapPair(pairB).token1(),
            "GothSwapBridgeMigrationRouter: Pair does not have one token matching"
        );
    }
    ...
    function _migrateToken(
        ...
        require(
            ...
            "GothSwapBridgeMigrationRouter: Didn't yield the correct amount"
        );
    }
    ...
    function migrateToken(
        ...
        require(
            ...
            "GothSwapBridgeMigrationRouter: migrator not registered"
        );
    ...
    function migrateLiquidityWithPermit(
        ...
        IGothSwapPair(liquidityPairFrom).permit(msg.sender, address(this), amount, deadline, v, r, s);
        _migrateLiquidity(
            liquidityPairFrom,
            liquidityPairTo,
            to,
    ...
    function _migrateLiquidity(
        ...
        address tokenToMigrate = IGothSwapPair(liquidityPairFrom).token0();
        if (
            IGothSwapPair(liquidityPairFrom).token0() == IGothSwapPair(liquidityPairTo).token0() ||
            IGothSwapPair(liquidityPairFrom).token0() == IGothSwapPair(liquidityPairTo).token1()
        ) {
            tokenToMigrate = IGothSwapPair(liquidityPairFrom).token1();
        }
        address newTokenAddress = bridgeMigrator[tokenToMigrate];
        require(
            newTokenAddress != address(0),
            "GothSwapBridgeMigrationRouter: Migrator not registered for the pair"
        );
        require(
            newTokenAddress == IGothSwapPair(liquidityPairTo).token0() ||
            newTokenAddress == IGothSwapPair(liquidityPairTo).token1(),
            "GothSwapBridgeMigrationRouter: Pair doesn't match the migration token"
        );

        (uint amountTokenA, uint amountTokenB) = _rescueLiquidity(liquidityPairFrom, amount);
        {
            uint amountToSwap = amountTokenA;
            if (tokenToMigrate != IGothSwapPair(liquidityPairFrom).token0()) {
                amountToSwap = amountTokenB;
            }
            _migrateToken(tokenToMigrate, amountToSwap);
        }
        if (IGothSwapPair(liquidityPairFrom).token0() != IGothSwapPair(liquidityPairTo).token0() &&
            IGothSwapPair(liquidityPairFrom).token1() != IGothSwapPair(liquidityPairTo).token1()
        ) {
        ...
        (uint changeAmount0, uint changeAmount1, ) = _addLiquidity(
            liquidityPairTo,
            IGothSwapPair(liquidityPairTo).token0(), IGothSwapPair(liquidityPairTo).token1(),
            amountTokenA, amountTokenB, to
        );
        if (changeAmount0 > 0) {
            TransferHelper.safeTransfer(IGothSwapPair(liquidityPairTo).token0(), to, changeAmount0);
        }
        if (changeAmount1 > 0) {
            TransferHelper.safeTransfer(IGothSwapPair(liquidityPairTo).token1(), to, changeAmount1);
        }
    }
    ...
    function _calculateRescueLiquidity(address pairAddress, uint amount) internal view returns (uint amount0, uint amount1) {
        (uint112 reserves0, uint112 reserves1, ) = IGothSwapPair(pairAddress).getReserves();
        uint totalSupply = IGothSwapPair(pairAddress).totalSupply();
    ...
    function calculateChargeBack(
        ...
        require(liquidityPairFrom != address(0), "GothSwapBridgeMigrationRouter: liquidityPairFrom address 0 not supported");
        require(liquidityPairTo != address(0), "GothSwapBridgeMigrationRouter: liquidityPairTo address 0 not supported");
        ...
        (uint112 reserve0, uint112 reserve1,) = IGothSwapPair(liquidityPairTo).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = GothSwapLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
